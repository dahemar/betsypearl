...
const CACHE_PATH = path.resolve(process.cwd(), '.cache/betsy-pearl-cms.json');
...
export async function fetchFromGoogleSheets(): Promise<SiteContent | null> {
  ...
  const [settingsRows, homeRows, portfolioRows, visualsRows, aboutRows, contactRows] =
  ...
  const bundle: SheetRowsBundle = {
    ...
    portfolio: await loadSheetRows(SHEET_TABS.portfolio, portfolioRows),
    visuals: await loadSheetRows(SHEET_TABS.visuals, visualsRows),
    about: await loadSheetRows(SHEET_TABS.about, aboutRows),
